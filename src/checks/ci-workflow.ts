...
export function checkCiWorkflow(ctx: LintContext): CheckResult[] {
  ...
  results.push({
    id: "A4",
    status:
      ci.includes("registry-url:") && /scope:\s*['"]@w3-io['"]/.test(ci)
        ? "pass"
        : "fail",
  ...
  results.push({
    id: "A6",
    status:
      ci.includes("npm run build") && !ci.includes("npm run package")
        ? "pass"
        : "fail",
