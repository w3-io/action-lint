...
export function checkManifest(ctx: LintContext): CheckResult[] {
  const manifest = ctx.readYaml("w3-action.yaml") as Manifest | null;
  ...
  results.push({
    ...
    label: "Every command has typed inputs",
    detail:
      commandsWithoutInputs.length > 0
  ...
  results.push({
    id: "M33",
    status:
      partner.length > 0 && /^[a-z0-9-]+$/.test(partner) ? "pass" : "fail",
