...
const HERMETIC_FIXTURE = resolve(import.meta.dirname, "../../test/fixtures/minimal-action");
...
const has = (slug) => existsSync(resolve(WORKSPACE, slug));
describe("runLint — hermetic fixture (runs in CI)", () => {
    ...
    it("every check has a well-formed shape (id, status, label)", () => {
        ...
        for (const c of allChecks) {
            ...
            assert.ok(["pass", "fail", "skip"].includes(c.status), `bad status: ${c.status} on ${c.id}`);
            assert.ok(typeof c.label === "string" && c.label.length > 0, `bad label on ${c.id}`);
        }
    });
    it("the fixture passes some checks (proves discovery works)", () => {
        const report = runLint(HERMETIC_FIXTURE);
        assert.ok(report.passed > 0, `expected some passes, got 0 — runner likely walked the wrong path`);
    });
});
describe("runLint — sibling action repos (skipped in CI)", () => {
    it("venice returns a valid report structure", { skip: !has("w3-venice-action") }, () => {
    ...
    it("venice passes all deterministic checks", { skip: !has("w3-venice-action") }, () => {
        const report = runLint(resolve(WORKSPACE, "w3-venice-action"));
    ...
    it("venice reports exactly 5 skipped checks", { skip: !has("w3-venice-action") }, () => {
        const report = runLint(resolve(WORKSPACE, "w3-venice-action"));
        assert.equal(report.skipped, 5);
    });
    it("aave passes all deterministic checks", { skip: !has("w3-aave-action") }, () => {
    ...
    it("stripe passes all deterministic checks", { skip: !has("w3-stripe-action") }, () => {
    ...
    it("sxt passes all deterministic checks", { skip: !has("w3-sxt-action") }, () => {
