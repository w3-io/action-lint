export function checkActionYml(ctx) {
    const action = ctx.readYaml("action.yml");
    const results = [];
    if (!action) {
        results.push({
            id: "E14",
    ...
    results.push({
        ...
        label: "action.yml runs.using: node24",
        detail: action.runs?.using === "node24"
            ? undefined
    ...
    results.push({
        id: "E15",
        status: outputKeys.length === 1 && outputKeys[0] === "result" ? "pass" : "fail",
        label: "Single result output",
        detail: outputKeys.length === 1 && outputKeys[0] === "result"
            ? undefined
    ...
    const srcContent = srcFiles
        ...
        .filter(Boolean)
        .join("\n");
    const hasGetInput = srcContent.includes("getInput") || srcContent.includes("requireInput");
    results.push({
        id: "K23",
